 ...
 * List of Parties
 */
exports.list = function(req, res) { Party.find().sort('-created').populate('user', 'displayName').exec(function(err, parties) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
 ...
 * Party middleware
 */
exports.partyByID = function(req, res, next, id) { Party.findById(id).populate('user', 'displayName').exec(function(err, party) {
