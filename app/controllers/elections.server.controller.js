 ...
 * List of Elections
 */
exports.list = function(req, res) { Election.find().sort('-created').populate('user', 'displayName').exec(function(err, elections) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
 ...
 * Election middleware
 */
exports.electionByID = function(req, res, next, id) { Election.findById(id).populate('user', 'displayName').exec(function(err, election) {
